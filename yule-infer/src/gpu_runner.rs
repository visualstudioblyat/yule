...
impl<'a> GpuTransformerRunner<'a> {
    ...
    fn forward(&mut self, token: u32) -> Result<Vec<f32>> {
        ...
        for layer in 0..self.cfg.n_layers {
            ...
            // Shared memory limit: 4096 seq_len (16KB at f32)
            if seq_len <= 4096 {
                let push = [
                    hd as u32,
                    seq_len,
                ...
                self.backend.dispatch_batched(
                    ...
                    push_bytes,
                    n_heads as u32, // one workgroup per head
                    1,
                    1,
                )?;
            } else {
                // Fallback: Flash-Decoding for very long sequences (>4096)
                for h in 0..n_heads {
                    let kv_h = h / kv_group;
